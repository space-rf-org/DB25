...
% Storage layer
            ...
            \node[htapbox] (unifstorage) at (1,0.5) {Unified Storage\\Manager};

% Output
            \node[box, fill=purple!20] (results) at (1,-1) {Query Results};
...
% Side annotations
            \node[align=left] at (-5,7.5) {\footnotesize \textbf{HTAP Routing}\\Workload Detection\\Query Classification};
            \node[align=left] at (-5,6) {\footnotesize \textbf{Parsing}\\AST Generation\\Schema Validation};
            \node[align=left] at (-5,4.5) {\footnotesize \textbf{Hybrid Planning}\\Dual Cost Models\\Format Selection};
            \node[align=left] at (-5,3) {\footnotesize \textbf{HTAP Execution}\\Row/Column Ops\\Comp. Storage};
            \node[align=left] at (-5,1) {\footnotesize \textbf{Unified Storage}\\Dual Formats\\Smart Devices};
