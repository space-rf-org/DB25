...
% Side annotations
\node[align=left] at (-3,6.5) {\footnotesize \textbf{Parsing}\\AST Generation\\Syntax Validation};
\node[align=left] at (-3,5) {\footnotesize \textbf{Optimization}\\Cost Estimation\\Plan Selection};
\node[align=left] at (-3,3.5) {\footnotesize \textbf{Execution}\\Vectorized Ops\\Parallel Processing};
...
\centering
\begin{tikzpicture}[
    layer/.style={rectangle, draw=black, thick, minimum width=12cm, minimum height=1.5cm, align=center, font=\large\bfseries},
...
\node[layer, fill=blue!20] (query_layer) at (0,5) {Query Processing Layer};
\node[component] (parser) at (-4,4.2) {Parser};
\node[component] (logical_planner) at (-1,4.2) {Logical Planner};
\node[component] (physical_planner) at (2,4.2) {Physical Planner};
\node[component] (optimizer) at (5,4.2) {Optimizer};

% Execution Layer
\node[layer, fill=green!20] (exec_layer) at (0,3) {Execution Layer};
\node[component] (scan_ops) at (-4,2.2) {Scan Operators};
\node[component] (join_ops) at (-1,2.2) {Join Operators};
\node[component] (sort_ops) at (2,2.2) {Sort Operators};
\node[component] (parallel_exec) at (5,2.2) {Parallel Executor};

% Storage Layer (Future)
\node[layer, fill=red!20, dashed] (storage_layer) at (0,1) {Storage Layer (Future Implementation)};
\node[future] (buffer_mgr) at (-4,0.2) {Buffer Manager};
\node[future] (page_mgr) at (-1,0.2) {Page Manager};
\node[future] (index_mgr) at (2,0.2) {Index Manager};
\node[future] (txn_mgr) at (5,0.2) {Transaction Mgr};

% Current vs Future indicators
\node[align=center, font=\footnotesize] at (-6.5,4) {\textbf{Current}\\Implementation};
\node[align=center, font=\footnotesize] at (-6.5,1) {\textbf{Future}\\Extensions};
...
\begin{tikzpicture}[
    ...
    data/.style={cylinder, draw=green, thick, minimum width=2cm, minimum height=1cm, align=center}
]

% Input
\node[data] (sql_input) at (0,8) {SQL Query\\String};

% Parsing steps
\node[process] (lexer) at (0,6.5) {Lexical\\Analysis};
\node[process] (parser) at (0,5) {Syntax\\Parsing};
\node[decision] (valid) at (0,3.5) {Valid\\AST?};
\node[process] (normalize) at (-3,2) {Query\\Normalization};
\node[process] (validate) at (3,2) {Schema\\Validation};
\node[data] (ast_output) at (0,0.5) {Validated\\AST};

% Error handling
\node[data, fill=red!20] (errors) at (-6,3.5) {Parse\\Errors};
...
% Slot directory
\node[font=\small] at (-3,2.2) {Slot Directory:};
\node[slot] at (-2,2) {0→2048};
\node[slot] at (-1,2) {1→1856};
\node[slot] at (0,2) {2→1664};
\node[slot] at (1,2) {3→1472};
\node[slot, fill=red!20] at (2,2) {4→DEL};

% Tuples
\node[tuple] at (-2,1) {Tuple 0\\(192 bytes)};
\node[tuple] at (0,1) {Tuple 1\\(192 bytes)};
\node[tuple] at (2,1) {Tuple 2\\(192 bytes)};
...
\begin{tikzpicture}[
    ...
    leaf/.style={rectangle, draw=green, thick, minimum width=1.5cm, minimum height=0.6cm, align=center, font=\tiny, fill=green!20},
    arrow/.style={->, thick}
]

% Root node
\node[internal] (root) at (0,4) {Root\\[25, 50]};

% Internal nodes
\node[internal] (int1) at (-3,2.5) {Internal\\[10, 20]};
\node[internal] (int2) at (0,2.5) {Internal\\[35, 40]};
\node[internal] (int3) at (3,2.5) {Internal\\[60, 70]};

% Leaf nodes
\node[leaf] (leaf1) at (-4.5,1) {[5,8,9]};
\node[leaf] (leaf2) at (-3,1) {[10,15]};
\node[leaf] (leaf3) at (-1.5,1) {[20,22,24]};
\node[leaf] (leaf4) at (-0.5,1) {[25,30,32]};
\node[leaf] (leaf5) at (0.5,1) {[35,37]};
\node[leaf] (leaf6) at (1.5,1) {[40,45,48]};
\node[leaf] (leaf7) at (2.5,1) {[50,55,58]};
\node[leaf] (leaf8) at (3.5,1) {[60,65]};
\node[leaf] (leaf9) at (4.5,1) {[70,75,80]};
