...
namespace db25 {
    ...
    public:
        ...
        [[nodiscard]] const PlannerConfig &get_config() const { return config_; }
        ...
        [[nodiscard]] std::vector<ExpressionPtr> extract_join_conditions_from_ast(const std::string& query) const;

        [[nodiscard]] ExpressionPtr build_subquery_expression(const nlohmann::json& sublink_node) const;

        // Helper for subquery types
        [[nodiscard]] SubqueryType determine_subquery_type(int sublink_type) const;

        // Enhanced AST-based WHERE condition extraction
        [[nodiscard]] std::vector<ExpressionPtr> extract_where_conditions_from_ast(const std::string& query) const;
    ...
    private:
        ...
        [[nodiscard]] LogicalPlanNodePtr build_join_node(LogicalPlanNodePtr left,
                                           LogicalPlanNodePtr right,
                                           JoinType join_type,
                                           const std::vector<ExpressionPtr> &conditions) const;

        LogicalPlanNodePtr build_projection_node(const LogicalPlanNodePtr &child,
                                                 const std::vector<ExpressionPtr> &projections);

        LogicalPlanNodePtr build_selection_node(const LogicalPlanNodePtr &child,
                                                const std::vector<ExpressionPtr> &conditions);

        LogicalPlanNodePtr build_aggregation_node(const LogicalPlanNodePtr &child,
                                                  const std::vector<ExpressionPtr> &group_by,
                                                  const std::vector<ExpressionPtr> &aggregations);

        // AST traversal helpers
        void extract_join_conditions_recursive(const nlohmann::json& node,
                                              std::vector<ExpressionPtr>& conditions) const;

        [[nodiscard]] ExpressionPtr parse_expression_from_ast(const nlohmann::json& expr_node) const;

        // Join type extraction
        [[nodiscard]] JoinType determine_join_type_from_ast(const nlohmann::json& join_node) const;

        [[nodiscard]] ExpressionPtr build_boolean_expression(const nlohmann::json &bool_expr_node) const;

        // Enhanced condition parsing
        [[nodiscard]] ExpressionPtr build_binary_operation(const nlohmann::json& a_expr_node) const;
        [[nodiscard]] ExpressionPtr build_column_reference(const nlohmann::json& column_ref_node) const;
        [[nodiscard]] ExpressionPtr build_constant_value(const nlohmann::json& const_node) const;
        [[nodiscard]] ExpressionPtr build_function_call(const nlohmann::json& func_call_node) const;
        ...
        [[nodiscard]] double estimate_join_cost(LogicalPlanNodePtr left, LogicalPlanNodePtr right,
                                  const std::vector<ExpressionPtr> &conditions) const;

        // Index selection
        [[nodiscard]] std::optional<std::string> select_best_index(const std::string &table_name,
                                                                   const std::vector<ExpressionPtr> &conditions) const;
        ...
        [[nodiscard]] bool can_use_index_for_condition(const ExpressionPtr &condition,
                                         const std::string &table_name,
                                         const std::string &index_name) const;

        // Cost estimation helpers
        [[nodiscard]] double estimate_table_scan_cost(const std::string &table_name) const;

        [[nodiscard]] double estimate_index_scan_cost(const std::string &table_name,
                                                      const std::string &index_name,
                                                      double selectivity) const;

        [[nodiscard]] double estimate_join_cost_internal(JoinType join_type,
                                                         size_t left_rows,
                                                         size_t right_rows,
                                                         double selectivity) const;
    };
    ...
    public:
        ...
        void traverse(const LogicalPlanNodePtr &node);
    };

    // Plan transformer for applying optimization rules
    class PlanTransformer : public PlanVisitor {
    public:
        LogicalPlanNodePtr transform(const LogicalPlanNodePtr &node);
